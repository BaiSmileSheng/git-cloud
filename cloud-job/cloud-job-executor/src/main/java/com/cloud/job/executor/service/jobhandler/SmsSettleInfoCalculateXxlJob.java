...
public class SmsSettleInfoCalculateXxlJob {
    ...
    public ReturnT<String> smsSettleInfoCalculateHandler(String param) throws Exception {
        ...
        TableDataInfo tableDataInfo = remoteSettleInfoService.list(smsSettleInfoReq);
        //2.计算加工费 (订单交货数量*sap加工费单价=生产订单加工费金额 )
        //3.修改
        if(null != tableDataInfo && CollectionUtils.isNotEmpty(tableDataInfo.getRows())){
            List<?> smsSettleInfoList = tableDataInfo.getRows();
            for(int i=0; i< smsSettleInfoList.size();i++){
                SmsSettleInfo smsSettleInfo = JSON.parseObject(JSON.toJSONString(smsSettleInfoList.get(i)), SmsSettleInfo.class);;
                smsSettleInfo.setSettlePrice(smsSettleInfo.getMachiningPrice().multiply(new BigDecimal(smsSettleInfo.getConfirmAmont())));
