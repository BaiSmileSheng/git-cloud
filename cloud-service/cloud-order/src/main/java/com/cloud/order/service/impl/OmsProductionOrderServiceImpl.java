...
public class OmsProductionOrderServiceImpl extends BaseServiceImpl<OmsProductionOrder> implements IOmsProductionOrderService {
    ...
    public R importProductOrder(List<OmsProductionOrderExportVo> list, SysUser sysUser) {
        ...
        outsourceStr.add("0");
        outsourceStr.add("1");
        List<OmsProductionOrderExportVo> outsourceOrderList = list.stream().filter((OmsProductionOrderExportVo o) ->
                outsourceStr.contains(o.getOutsourceType())).collect(toList());
        //获取导入的加工承揽的但不允许加工承揽的排产订单,需返回给用户
        List<OmsProductionOrderExportVo> noOutsourceOrderList = outsourceOrderList.stream().filter((OmsProductionOrderExportVo o) ->
        ...
        //需导入数据
        List<OmsProductionOrderExportVo> omsProductionOrderExportVos = list.stream()
        ...
        //匹配UPH节拍数据
        omsProductionOrderExportVos.forEach(o -> materialInfoList.forEach(m -> {
        ...
        //筛选没有UPH节拍的数据
        List<OmsProductionOrderExportVo> noUphProductOrders = omsProductionOrderExportVos.stream()
        ...
        omsProductionOrderExportVos = omsProductionOrderExportVos.stream()
                .filter(item -> !noUphProductOrders.contains(item)).collect(toList());
        //计算用时：排产量/UPH节拍
        omsProductionOrderExportVos.forEach(o ->
        ...
        //匹配产品定员信息,分公司主管、班长
        omsProductionOrderExportVos.forEach(o -> cdFactoryLineInfoList.forEach(f -> {
        ...
        List<OmsProductionOrderExportVo> noQuotaProductOrders = omsProductionOrderExportVos.stream()
                .filter(o ->
        ...
        //匹配生命周期
        omsProductionOrderExportVos.forEach(o -> materialExtendInfoList.forEach(m -> {
            if (o.getProductMaterialCode().equals(m.getMaterialCode())) {
                o.setLifeCycle(m.getLifeCycle());
            }
        }));
        //筛选没有生命周期的数据
        List<OmsProductionOrderExportVo> noLifeCycleProductOrders = omsProductionOrderExportVos.stream()
        ...
        List<OmsProductionOrderExportVo> checkOrderStatus = deleteOldProductOrder(omsProductionOrderExportVos);
        ...
        List<OmsProductionOrder> omsProductionOrders = omsProductionOrderExportVos.stream().map(o -> {
        ...
        //组织缺基础数据的订单
        List<OmsProductionOrderExportVo> exportProductOrder = new ArrayList<>();
        ...
        if (exportProductOrder.size() > 0) {
            return EasyExcelUtilOSS.writeExcel(exportProductOrder, "排产订单.xlsx", "sheet", new OmsProductionOrderExportVo());
        } else {
            return R.ok();
        }
    }
    ...
    public List<OmsProductionOrder> exportAll(OmsProductionOrder omsProductionOrder, SysUser sysUser) {
        ...
        List<OmsProductionOrderExportVo> productionOrderVos = omsProductionOrderList.stream().map(o -> {
            OmsProductionOrderExportVo omsProductionOrderExportVo = new OmsProductionOrderExportVo();
            BeanUtils.copyProperties(o, omsProductionOrderExportVo);
    ...
    private List<OmsProductionOrder> checkOverdueNotCloseOrder(List<OmsProductionOrder> list, SysUser sysUser) {
        ...
        if (checkOrders.size() > 0) {
            //  超期未关闭订单审批流程
            checkOrders.forEach(omsProductionOrder -> {
