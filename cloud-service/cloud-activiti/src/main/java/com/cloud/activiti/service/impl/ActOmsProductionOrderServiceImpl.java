...
public class ActOmsProductionOrderServiceImpl implements IActOmsProductionOrderService {
    ...
    public R timeCheckProductOrderAct() {
        ...
        BigDecimal countSum = new BigDecimal(omsProductionOrders.size());
        BigDecimal size = new BigDecimal("3");
        BigDecimal count = countSum.divide(size,BigDecimal.ROUND_UP);
        ObjectMapper objectMapper = new ObjectMapper();
        List<List<OmsProductionOrder>> orderList =
                objectMapper.convertValue(ListCommonUtil.subCollection(omsProductionOrders, count.intValue()),
                        new TypeReference<List<List<OmsProductionOrder>>>() {
                        });
            ...
            threadPoolTaskExecutor.newThread(new Runnable() {
                ...
                public void run() {
                    ...
                    if (!detailMap.isSuccess()) {
                        ...
                        throw new BusinessException("定时任务开启排产订单审批流程，查询排产订单明细失败！");
                    }
                    ...
                    R businessVoMap = remoteProductionOrderService.checkProductOrderAct(orders);
                    if (!businessVoMap.isSuccess()) {
                        log.error("调用order服务，校验排产订单的审批闸口失败，原因：" + businessVoMap.get("msg"));
                        throw new BusinessException("调用order服务，校验排产订单的审批闸口失败，原因：" + businessVoMap.get("msg"));
                    }
                    ...
                    if (ObjectUtil.isEmpty(businessVoList) || businessVoList.size() <= 0) {
                        ...
                        R updateOrderMap = remoteProductionOrderService.updateBatchByPrimary(orders);
                    ...
                    if (!startActMap.isSuccess()) {
                        ...
                        throw new BusinessException("定时任务开启排产订单审批流程失败，原因：" + startActMap.get("msg"));
                    }
                    //4、更新排产订单的状态
